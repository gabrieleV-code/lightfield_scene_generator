# Run as:
# "C:\Program Files\Blender Foundation\Blender 3.3\blender.exe" -b "C:\Users\gabri\OneDrive\Documenti\UniTO-DOCS\Stage\LightfieldsRenderer\lightfield_objaverse_3_36_BiggerScale.blend" -P main2.py -- --name test

#This script is used to call a rendering on a already settled scene
...
def main(config: dict):
    ...
    config["camera"]["dof"]=1.5
    config["camera"]["distance"]=1.5
    full_generation_start = time.time()

    number_of_renders = 10
    scene_name = "Scene_"+str(scene_idx)
    from src.blender_utilities import init_for_render
