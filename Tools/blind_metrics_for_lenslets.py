...
def multi_single_image_metrics(mypath):
	onlyfiles = [ f for f in listdir(mypath) if isfile(join(mypath,f)) and re.search(r'\.png', f)] #and re.search(r'input_', f)
	for n in range(0, len(onlyfiles)):
...
def main():
	path = r'C:\Users\gabri\OneDrive\Documenti\UniTO-DOCS\Stage\LightfieldsRenderer\Lightfield_Padded_001\Lenslets'
	array_paths = descend_folder(path,[],'Lenslets')
...
if __name__ == "__main__": 
	#main() 
	path = r'C:\Users\gabri\OneDrive\Documenti\UniTO-DOCS\Stage\LightfieldsRenderer\Lightfield_Padded_001\Lenslets'
	multi_single_image_metrics(path)
