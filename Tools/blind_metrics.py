...
def angular_PSNR(img_set):
	...
	print(f"PSNR value is {psnr_mean} dB") 
	return psnr_angular
...
def angular_SSIM(img_set):
	...
	psnr_angular = 1/(img_center.shape[0]*img_center.shape[1]) * psnr_mean
	print(f"SSMI value is {psnr_mean} dB") 
	return psnr_angular
...
def multi_metrics(path): 
	img_set = load_images(path)
	value = angular_PSNR(img_set) 
	#print(f"Ang PSNR value is {value} dB") 
...
def main():
	path = r'C:\Users\gabri\OneDrive\Documenti\UniTO-DOCS\Stage\LightfieldsRenderer\Lightfield_001\LFs\LF_Scene_02'
	array_paths = descend_folder(path,[],'png')
	for p in array_paths:
		print(f"Calculating metrics for {p}: ")
		multi_metrics(p)
